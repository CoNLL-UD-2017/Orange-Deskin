...
struct LookupParameters : public ParametersBase {
 ...
 private:
  ...
  void load(Archive& ar, const unsigned int) {
    ...
    //for (unsigned i = 0; i < values.size(); ++i)  ar & values[i];

    // JHE we load nv vectors from the model (but we have allocated more than that, since we want to add some new values
    if (nv != values.size()) std::cerr << "Attention " << nv << " != " << values.size() << std::endl;
    for (unsigned i = 0; i < nv; ++i)   ar & values[i];
  }
